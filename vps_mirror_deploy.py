...
DEPLOY_LIST = [
    ...
    # Edelweiss (Vision Play) - create directory
    (f"{LOCAL_ROOT}/www/edelweiss/index.html", f"{REMOTE_BASE}/edelweiss/index.html"),
]

# Redesigned app pages (overwrite dist index.html with unified design)
REDESIGNED_APPS = {
    f"{LOCAL_ROOT}/www/iaputa/index.html": f"{REMOTE_BASE}/iaputa/index.html",
    f"{LOCAL_ROOT}/www/nikolina/index.html": f"{REMOTE_BASE}/nikolina/index.html",
    f"{LOCAL_ROOT}/www/traductor/index.html": f"{REMOTE_BASE}/traductor/index.html",
    f"{LOCAL_ROOT}/www/taskflow/index.html": f"{REMOTE_BASE}/taskflow/index.html",
    f"{LOCAL_ROOT}/www/dohler/index.html": f"{REMOTE_BASE}/dohler/index.html",
    f"{LOCAL_ROOT}/www/logisearch/index.html": f"{REMOTE_BASE}/logisearch/index.html",
    f"{LOCAL_ROOT}/www/edelweiss/index.html": f"{REMOTE_BASE}/edelweiss/index.html",
    f"{LOCAL_ROOT}/www/moko/index.html": f"{REMOTE_BASE}/moko/index.html",
    f"{LOCAL_ROOT}/www/combipro/index.html": f"{REMOTE_BASE}/combipro/index.html",
}
...
def build_projects():
    ...
    for d, cmd in build_commands:
        if os.path.exists(d):
            ...
            try:
                subprocess.run(
                    cmd, cwd=d, shell=True, check=True, stdout=subprocess.DEVNULL
                )
...
def main():
        ...
        if os.path.isfile(local_source):
            ...
            for root, dirs, files in os.walk(local_source):
                ...
                remote_dir = (
                    remote_dest
                    if rel_path == "."
                    else f"{remote_dest}/{rel_path.replace(os.sep, '/')}"
                )
    ...
    # Upload redesigned app pages (overwrite dist index.html with unified design)
    print("\n🎨 Subiendo páginas rediseñadas...")
    for local_redesigned, remote_redesigned in REDESIGNED_APPS.items():
        if os.path.isfile(local_redesigned):
            print(f"  Rediseñando: {remote_redesigned}")
            upload_file(ftp, local_redesigned, remote_redesigned)
        else:
            print(f"  ⚠️ No encontrado: {local_redesigned}")
